...
<fieldset>
    ...
    <?php echo $form->textFieldRow($model, 'email', array('prepend' => '<i class="icon-envelope"></i>', 'placeholder' => "Vào địa chỉ email@qcdn.com", 'class' => 'span5', 'autocomplete' => 'off', 'maxlength' => 128, 'disabled' => $model->isNewRecord?false:true)); ?>
    ...
    <?php echo $form->dropDownListRow($model, 'default_role', Lookup::items('AccessRole'), 
            array('hint' => 'Thêm quyền truy cập cho người dùng mới.',
        'prompt' => 'Chọn quyền truy cập',
//        'ajax' => array(
//            'type' => 'POST',
...
<?php if (!$model->isNewRecord): ?>
    <div class="control-group ">
        ...
        <div class="controls">
            <?php
            if ((!empty($model->old_avatar) || $model->old_avatar != "") && (file_exists(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . User::TYPE . DIRECTORY_SEPARATOR . $model->old_avatar))) {
                $model->thumbnail = Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias("uploads") . DIRECTORY_SEPARATOR . User::TYPE . DIRECTORY_SEPARATOR . "thumbnail" . DIRECTORY_SEPARATOR . ImageHelper::GetThumbnail($model->old_avatar, User::TYPE, "240x180"));
            } else {
