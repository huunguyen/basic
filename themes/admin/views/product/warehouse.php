...
<div class="widget">
    ...
    $this->widget('bootstrap.widgets.TbGridView', array(
        ...
        'id' => 'product-warehouse-grid',
        'dataProvider' => $warehouse->searchByProduct($model->id_product),
        'pagerCssClass' => 'pagination pagination-right',
        'template' => '{summary}{items}{pager}',
        'enablePagination' => true,
        'summaryText' => 'Tất cả <b>Nhà Kho</b> cho Sản phẩm [<b>' . $model->name . '</b>] Hiển thị từ {start}-{end} của {count} kết quả.',
        'columns' => array(
            array(
                'name' => 'logo',
                'header' => 'Ảnh',
                'type' => 'html',
                'value' => 'CHtml::link(CHtml::image($data->thumbnail,"",array("class"=>"img-rounded", "style"=>"width:50px;height:50px;")), Yii::app()->createUrl("warehouse/view", array("id"=>$data["id_warehouse"])), array("class"=>"highslide", "rel"=>"myrel"))',
                'htmlOptions' => array('style' => 'width: 50px')
            ),
            array('name' => 'name', 'header' => 'Tên NCC', 'value' => '$data["name"]."[".$data["id_warehouse"]."]"'),
            array('name' => 'date_add', 'header' => 'Ngày Tạo NCC'),
            array(
                'class' => 'bootstrap.widgets.TbButtonColumn',
                'header' => 'Quản trị',
                'template' => '{addWarehouse}',
                'buttons' => array(
                    'addWarehouse' => array(
                        'label' => 'Hủy Nhà Cung Cấp với Sản phẩm',
                        'icon' => 'minus',
                        //'imageUrl' => Yii::app()->request->baseUrl . '/images/icons/usual/icon-dialog.png',
                        'url' => 'Yii::app()->createUrl("product/deleteWarehouse", array("id_product"=>' . $model->id_product . ', "id_warehouse"=>$data["id_warehouse"]) )',
                        'click' => "function() {
                        ...
                        $.fn.yiiGridView.update('warehouse-grid', {
                            ...
                            success:function(data) {
                                console.log(data);                                
                                $.fn.yiiGridView.update('product-warehouse-grid');
                                $.fn.yiiGridView.update('warehouse-grid');
                            },
                            error:function(data) {
                                console.log(data);                                
...
<fieldset>    
    ...
    <div class="widget">
        ...
        $this->widget('bootstrap.widgets.TbGridView', array(
            'type' => 'striped bordered condensed',
            'id' => 'warehouse-grid',
            'dataProvider' => $warehouse->searchByNoProduct($model->id_product),
            'pagerCssClass' => 'pagination pagination-right',
            'template' => '{summary}{items}{pager}',
            'enablePagination' => true,
            ...
            'columns' => array(
                ...
                array('name' => 'name', 'header' => 'Tên Kho Hàng', 'value' => '$data["name"]."[".$data["id_warehouse"]."]"'),
                ...
                array(
                    ...
                    'buttons' => array(
                        'addWarehouse' => array(
                            'label' => 'Thêm Nhà Kho Cho Tất Cả [Sản phẩm: ' . $model->name . ']',
                            'icon' => 'plus',
                            //'imageUrl' => Yii::app()->request->baseUrl . '/images/icons/usual/icon-dialog.png',
                            'url' => 'Yii::app()->createUrl("product/addWarehouse", array("id_product"=>' . $model->id_product . ', "id_warehouse"=>$data["id_warehouse"]) )',
                            'click' => "function() {
        ...
        if (isset($model->productAttributes) && count($model->productAttributes) > 0) {
            $this->widget('bootstrap.widgets.TbGridView', array(
                ...
                'columns' => array(
                    ...
                    array('name' => 'warehouse_reference', 'header' => 'Tham khảo NCC'),
                    array(
                        'class' => 'bootstrap.widgets.TbButtonColumn',
                        'header' => 'Quản lý',
...
<span class="clear"></span>
    <div class="form-actions">   
        <?php
        echo CHtml::ajaxSubmitButton('Thêm Kho Hàng Cho Các Mặc Hàng', CHtml::normalizeUrl(array('product/addWarehouse', 'id_product' => $model->id_product)), array(
            ...
            //'dataType' => 'json',
            'cache' => 'false'
                ), array('class' => 'buttonS bLightBlue', 'id' => 'save-warehouse' . uniqid()));
        ?>
    </div>
</fieldset>
<?php $this->endWidget(); ?>  
