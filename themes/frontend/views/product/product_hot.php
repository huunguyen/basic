<div id="title">SẢN PHẨM ƯU CHUỘNG </div>
<div id = 'ajaxRowhot'>
    <?php $i = 1; ?>
        ...
        <?php } else { ?>
            <div id="content2_l" style="margin-right:25px">
                <div id="content2_k">
                    <ul> 
                        <li>
                            <a href="<?= Yii::app()->createUrl('product/view', array('id' => $value->id_product)) ?>"> <img src="<?= $image ?>" alt="<?php echo $value->meta_title; ?>" /></a>
                        </li>
                    </ul>
                    <ul style="padding-left:10px;">
                        <li><h1><?php echo StringHelper::Limit($value->name, 15); ?></h1></li>
...
<script>
    jQuery("body").delegate("#pagehot a", "click", function() {
        $("#attribute0").css("display", "block");
        var qUrl = jQuery(this).attr("href");
        jQuery.get(qUrl, function(data) {
            jQuery('#ajaxRowhot').html($(data).find('#ajaxRowhot').html());
