            ...
            if (!empty($data)) {
                foreach ($data as $key => $value):
                    ...
                    <tr>
                        ...
                        <td class="fileInfo">
                            <input id="quanty_<?= $key; ?>" style="width:50px;float: left;margin-left:20px" class="select_number" type="number" value="<?php echo $value['soluong']; ?>">
                    ...
                    foreach ($data_pro as $value):
                        ...
                        <tr>
                            ...
                            <td>
                                <?php
                                echo CHtml::ajaxLink(
                                        '<img src="' . Yii::app()->baseUrl . '/images/delete.png" title="Xóa sản phẩm khỏi giỏ hàng">', Yii::app()->createUrl('product/deletepack'), array(
                                    'type' => 'POST',
                                    'async' => false,
                                    'data' => array('key' => $key),
                                    'update' => '#cart_sh'
                                        ), array(
                                    'href' => Yii::app()->createUrl('product/deletepack'),
                                        )
                                )
                                ?>
                            </td>
...
<script>
    ...
    function delete_cart(key) {
        $.ajax({
            url: "<?= Yii::app()->createUrl('product/deletecart') ?>",
            type: 'POST',
            data: 'key=' + key,
            async: false,
            beforeSend: function(xhr) {
