...
class CartController extends Controller {
    ...
    public function actionView($id) {
        $price = new SpecificPrice('searchByCart');
        $price->unsetAttributes();  // clear any default values
        // This portion of code is belongs to Page size dropdown.
        // This portion of code is belongs to Page size dropdown.
        $uni_id = ImageHelper::encrypt_text(Yii::app()->controller->id . '@' . Yii::app()->controller->action->id . '@' . get_class($price));
        $pageSize = Yii::app()->user->getState($uni_id . lcfirst(get_class($price)) . 'PageSize', Yii::app()->params['defaultPageSize']);

        if (isset($_GET['pageSize'])) {
            $pageSize = (int) $_GET['pageSize'];
            Yii::app()->user->setState($uni_id . lcfirst(get_class($price)) . 'PageSize', $pageSize);
            unset($_GET['pageSize']);
        }
        if (isset($_GET[get_class($price) . '_page'])) {
            $newsPage = (int) $_GET[get_class($price) . '_page'] - 1;
            Yii::app()->user->setState(get_class($price) . '_page', $newsPage);
            unset($_GET[get_class($price) . '_page']);
        } else if (isset($_GET['ajax'])) {
            Yii::app()->user->setState(get_class($price) . '_page', 0);
        }
...
// if AJAX request (triggered by deletion via admin grid view), we should not redirect the browser
        ...
        elseif($model = $this->loadModel($id)){
            $model->delete();
            if (!isset($_GET['ajax']))
                $this->redirect(isset($_POST['returnUrl']) ? $_POST['returnUrl'] : array('orders/index'));
        }
        else
