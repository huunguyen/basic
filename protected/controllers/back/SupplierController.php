    ...
    public function accessRules() {
        return array(
            array('allow', // allow all users to perform 'index' and 'view' actions
                'actions' => array('index', 'view', 'create', 'update', 'products'),
                'users' => array('*'),
            ),
  ...
  public function actionProducts() {
        if (Yii::app()->request->isPostRequest) {
            ...
            foreach ($ps as $p) {
                ...
                if (($product != null) && isset($product->productAttributes)) {
    ...
    protected function resizePhoto($fileName, $width, $height, $inputPath = null, $outputPath = null) {
        ...
        if (in_array($ext, $upload_permitted_image_types)) {
            $inputPath = Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Supplier::TYPE . DIRECTORY_SEPARATOR . $fileName;
            $outputDirectory = Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Supplier::TYPE . DIRECTORY_SEPARATOR . 'thumbnail';
            // check exist location if not exist to create location
            if (!file_exists(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Supplier::TYPE . DIRECTORY_SEPARATOR . 'thumbnail')) {
                if (!file_exists(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Supplier::TYPE)) {
                    mkdir(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Supplier::TYPE, 0777);
                    mkdir(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Supplier::TYPE . DIRECTORY_SEPARATOR . 'thumbnail', 0777);
                }
                else
                    mkdir(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Supplier::TYPE . DIRECTORY_SEPARATOR . 'thumbnail', 0777);
            }
    ...
    protected function savePhoto($uploadedFile, $fileName) {
        if (!empty($uploadedFile) & is_object($uploadedFile)) {
            if(!is_writeable(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Supplier::TYPE))
                    chmod(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Supplier::TYPE, 0777);
            $uploadedFile->saveAs(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Supplier::TYPE . DIRECTORY_SEPARATOR . $fileName);
            chmod(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Supplier::TYPE . DIRECTORY_SEPARATOR . $fileName, 0777);
...
// Uncomment the following line if AJAX validation is needed
        ...
        if (isset($_POST['Supplier'])) {
            ...
            $uploadedFile2 = CUploadedFile::getInstance($model, 'certificate');
            
            $fileName = preg_replace('/[.][.]+/s', '.', preg_replace('/[^a-zA-Z0-9.]/s', '', "{$rnd}_{$uploadedFile}"));
            $fileName2 = preg_replace('/[.][.]+/s', '.', preg_replace('/[^a-zA-Z0-9.]/s', '', "cer_{$rnd}_{$uploadedFile2}"));
            
            $model->attributes = $_POST['Supplier'];            
            if (is_object($uploadedFile) && !$uploadedFile->hasError)
                $model->logo = $fileName;
            if (is_object($uploadedFile2) && !$uploadedFile2->hasError)
                $model->certificate = $fileName2;
            ...
            if ($model->validate()) {
                ...
                try {
                    if ($model->save(false)) {   // modify existing line to pass in false param
                        ...
                        if (is_object($uploadedFile2)) {
                            // check exist location if not exist to create location
                            if (!file_exists(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Supplier::TYPE)){
                                mkdir(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Supplier::TYPE, 0777);  
                                mkdir(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Supplier::TYPE . DIRECTORY_SEPARATOR . 'thumbnail', 0777); 
                            }  
                            $fileName2 = 'cer_'.$model->getPrimaryKey().'.'.ImageHelper::FilenameExtension($fileName2); 
                            $this->savePhoto($uploadedFile, $fileName);
                        }
                        
                        $transaction->commit();
                        ImageHelper::delOtherImage(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Supplier::TYPE, $model->getPrimaryKey(), ImageHelper::FilenameExtension($fileName));
                        $model = Supplier::model()->findByPk($model->getPrimaryKey());                         
                        $model->updateByPk($model->id_supplier, array('logo' => $fileName));
                        Yii::app()->user->setFlash('success', 'đã được tạo <strong>Thành công! </strong>');                        
                        $this->redirect(array('supplier/index', 'id' => $model->id_supplier));
                    } else {
...
// Uncomment the following line if AJAX validation is needed
 $this->performAjaxValidation($model);
        ...
        if (isset($_POST['Supplier'])) {
            ...
            if (is_object($uploadedFile) && !$uploadedFile->hasError)
                $model->logo = $fileName = $model->id_supplier.'.'.ImageHelper::FilenameExtension($fileName);
            if (is_object($uploadedFile2) && !$uploadedFile2->hasError)
                $model->certificate = $fileName2  = 'cer_'.$model->id_supplier.'.'.ImageHelper::FilenameExtension($fileName2);
            
            if((is_null($model->logo) || empty($model->logo)) && isset($model->old_logo)) $model->logo = $model->old_logo;
                ...
                try {                    
                    if ($model->save(false)) {   // modify existing line to pass in false param
                        ...
                        if (is_object($uploadedFile2)) {
                            ...
                            $this->savePhoto($uploadedFile2, $fileName2);
                        }
...
// if AJAX request (triggered by deletion via admin grid view), we should not redirect the browser
            ...
            Supplier::model()->updateByPk($id, array('active' => 0, 'date_upd' => new CDbExpression('NOW()')));
    }
