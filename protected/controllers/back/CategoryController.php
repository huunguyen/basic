...
class CategoryController extends Controller {
    ...
    protected function resizePhoto($fileName, $width, $height, $inputPath = null, $outputPath = null) {
        ...
        if (in_array($ext, $upload_permitted_image_types)) {
            $inputPath = Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Category::TYPE . DIRECTORY_SEPARATOR . $fileName;
            $outputDirectory = Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Category::TYPE . DIRECTORY_SEPARATOR . 'thumbnail';
            // check exist location if not exist to create location
            if (!file_exists(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Category::TYPE . DIRECTORY_SEPARATOR . 'thumbnail')) {
                if (!file_exists(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Category::TYPE)) {
                    mkdir(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Category::TYPE, 0777);
                    mkdir(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Category::TYPE . DIRECTORY_SEPARATOR . 'thumbnail', 0777);
                }
                else
                    mkdir(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Category::TYPE . DIRECTORY_SEPARATOR . 'thumbnail', 0777);
            }
    ...
    protected function savePhoto($uploadedFile, $fileName) {
        if (!empty($uploadedFile) & is_object($uploadedFile)) {
            if (!is_writeable(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Category::TYPE))
                chmod(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Category::TYPE, 0777);
            $uploadedFile->saveAs(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Category::TYPE . DIRECTORY_SEPARATOR . $fileName);
            chmod(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Category::TYPE . DIRECTORY_SEPARATOR . $fileName, 0777);
            ...
            if ($model->validate()) {
                ...
                try {
                    if ($model->save(false)) {   // modify existing line to pass in false param
                        ...
                        $transaction->commit();
                        ImageHelper::delOtherImage(Yii::getPathOfAlias('uploads') . DIRECTORY_SEPARATOR . Category::TYPE, $model->getPrimaryKey(), ImageHelper::FilenameExtension($fileName));
...
//                        $model->updateByPk($model->id_manufacturer, array('logo' => $fileName));
                        ...
                        $this->redirect(array('category/index', 'id' => $model->id_category));
...
// Uncomment the following line if AJAX validation is needed
        ...
        if (isset($_POST['Category'])) {
            ...
            $model->attributes = $_POST['Category'];
            if (is_object($uploadedFile) && !$uploadedFile->hasError)
                $model->img = $fileName = $model->id_category . '.' . ImageHelper::FilenameExtension($fileName);
            if ($model->validate()) {
                $transaction = Yii::app()->db->beginTransaction();
                try {
                    if ($model->save(false)) {   // modify existing line to pass in false param
...
// if AJAX request (triggered by deletion via admin grid view), we should not redirect the browser
            ...
            Category::model()->updateByPk($id, array('active' => 0, 'date_upd' => new CDbExpression('NOW()')));
