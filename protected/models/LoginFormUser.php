...
class LoginFormUser extends CFormModel {
    ...
    public function rules() {
        return array(
            ...
            array('verifyCode', 'captcha', 'allowEmpty' => !CCaptcha::checkRequirements(), 'message' => "<b>{attribute}</b> không chính xác."),
        ...
        if (!$this->hasErrors()) {
            ...
            switch ($this->_identity->errorCode) {
                ...
                case UserIdentity::ERROR_EMAIL_INVALID:
                    $this->addError("password", 'Mật khẩu hoặc Tên người dùng không chính xác.');
                    break;
    ...
    public function login() {
        ...
        if ($this->_identity->errorCode === UserIdentity::ERROR_NONE) {
            $duration = $this->rememberMe ? 3600 * 24 * 30 : 0; // 30 days
            Yii::app()->user->login($this->_identity, $duration);
            User::model()->updateByPk($this->_identity->id, array('validation_key' => md5(mt_rand() . mt_rand() . mt_rand()), 'login_time' => new CDbExpression('NOW()')));
            return true;
        }
        else
            return false;
