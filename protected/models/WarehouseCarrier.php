...
/**
 * This is the model class for table "tbl_warehouse_carrier".
 *
 * The followings are the available columns in table 'tbl_warehouse_carrier':
 * @property string $id_carrier
 * @property string $id_warehouse
 */
class WarehouseCarrier extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('id_carrier, id_warehouse', 'required'),
			array('id_carrier, id_warehouse', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id_carrier, id_warehouse', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return WarehouseCarrier the static model class
	 */
    ...
    public function updateRecord() {
        $model = self::model()->findByPk(array('id_carrier' => $this->id_carrier, 'id_warehouse' => $this->id_warehouse));
