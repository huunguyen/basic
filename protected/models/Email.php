<?php

/**
 * This is the model class for table "tbl_email".
 *
 * The followings are the available columns in table 'tbl_email':
 * @property string $id_email
 * @property string $from
 * @property string $to
 * @property string $subject
 * @property string $body
 * @property string $sendtime
 * @property string $status
 * @property string $id_template_email
 * @property string $id_action
 * @property string $id_user
 */
class Email extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return Email the static model class
	 */
	public static function model($className=__CLASS__)
	{
		return parent::model($className);
	}

	/**
	 * @return string the associated database table name
	 */
	public function tableName()
	{
		return 'tbl_email';
	}

	/**
	 * @return array validation rules for model attributes.
	 */
	public function rules()
	{
		// NOTE: you should only define rules for those attributes that
		// will receive user inputs.
		return array(
			array('id_template_email, id_action, id_user', 'required'),
			array('from, to, subject, body, status', 'length', 'max'=>45),
			array('id_template_email, id_action, id_user', 'length', 'max'=>10),
			array('sendtime', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id_email, from, to, subject, body, sendtime, status, id_template_email, id_action, id_user', 'safe', 'on'=>'search'),
		);
	}

	/**
	 * @return array relational rules.
	 */
	public function relations()
	{
		// NOTE: you may need to adjust the relation name and the related
		// class name for the relations automatically generated below.
		return array(
		);
	}

	/**
	 * @return array customized attribute labels (name=>label)
	 */
	public function attributeLabels()
	{
		return array(
			'id_email' => 'Id Email',
			'from' => 'From',
			'to' => 'To',
			'subject' => 'Subject',
			'body' => 'Body',
			'sendtime' => 'Sendtime',
			'status' => 'Status',
			'id_template_email' => 'Id Template Email',
			'id_action' => 'Id Action',
			'id_user' => 'Id User',
		);
	}

	/**
	 * Retrieves a list of models based on the current search/filter conditions.
	 * @return CActiveDataProvider the data provider that can return the models based on the search/filter conditions.
	 */
	public function search()
	{
		// Warning: Please modify the following code to remove attributes that
		// should not be searched.

		$criteria=new CDbCriteria;

		$criteria->compare('id_email',$this->id_email,true);
		$criteria->compare('from',$this->from,true);
		$criteria->compare('to',$this->to,true);
		$criteria->compare('subject',$this->subject,true);
		$criteria->compare('body',$this->body,true);
		$criteria->compare('sendtime',$this->sendtime,true);
		$criteria->compare('status',$this->status,true);
		$criteria->compare('id_template_email',$this->id_template_email,true);
		$criteria->compare('id_action',$this->id_action,true);
		$criteria->compare('id_user',$this->id_user,true);

		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
		));
	}
}